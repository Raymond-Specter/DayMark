    ...
    <>
      <div className="section-toolbar">
        ...
        <div className="date-range">
          <label>
            <span className="sr-only">统计开始日期</span>
            <input
              type="date"
              value={range.start}
          ...
          <span>—</span>
          <label>
            <span className="sr-only">统计结束日期</span>
            <input
              type="date"
              value={range.end}
      ...
      <div className="metric-grid dashboard-metrics">
        {[
          ...
          },
          {
            ...
            unit: "%",
            icon: BarChart3,
            note: `${stats.completed_tasks} / ${stats.total_tasks} 项任务`,
          },
          {
            label: "计划 / 实际",
      ...
      <div className="insights-grid">
        <section className="panel chart-panel">
          ...
          <div className="bar-chart" aria-label="每日计划与实际时间柱状图">
            {stats.daily.length ? (
              stats.daily.map((day) => (
                <div
                  className="chart-column"
                  key={day.date}
                  title={`${day.date}：计划 ${day.planned_minutes} 分钟，实际 ${day.actual_minutes} 分钟`}
                >
              ...
              <div className="chart-empty">所选时间范围暂无记录</div>
            )}
          </div>
          <div className="chart-baseline">
            <span>0 h</span>
            <span>纵轴最大值 {hours(maxMinutes)} h</span>
          </div>
        </section>
        <section className="panel distribution-panel">
          <div className="panel-heading">
            <div>
              <span className="eyebrow">WHERE YOUR TIME GOES</span>
              <h2>项目时间分布</h2>
            </div>
            <FolderOpen size={19} />
          </div>
          {stats.project_distribution.length ? (
            stats.project_distribution.map((p) => (
              ...
              >
                <div>
                  <span>
                    <i style={{ backgroundColor: p.color || "#5369ed" }} />
                    {p.name}
                  </span>
                  <strong>
                    {hours(p.actual_minutes)} h <small>实际</small>
                  </strong>
                </div>
                <ProgressBar
                  value={
                  ...
                  color={p.color}
                />
                <p>计划 {hours(p.planned_minutes)} h</p>
              </div>
            ))
          ) : (
      ...
      <section className="panel">
        ...
        {stats.upcoming_deadlines.length ? (
          ...
          <p className="empty-line">当前没有临近的截止事项。</p>
        )}
      </section>
    </>
  );
...
export function DailyReview({
  ...
  const energy = ["有点疲惫", "状态偏低", "平稳", "状态不错", "精力充沛"];
  return (
    <>
      <div className="section-toolbar">
        <span className="subtle-note">
        ...
        <label className="date-range">
          <span className="sr-only">复盘日期</span>
          <input
            type="date"
            max={data.today}
            value={date}
      ...
      {!review ? (
        <div className="panel loading-panel">
          {error ? "复盘加载失败，请重新选择日期重试。" : "正在读取当天记录…"}
        </div>
      ) : (
          ...
          <form className="panel review-form" onSubmit={save}>
            ...
            <label className="field">
              ...
              <div className="minutes-input">
                ...
                />
                <span>分钟</span>
                <small>{hours(review.actual_minutes)} 小时</small>
              </div>
            </label>
            <fieldset className="energy-field">
              <legend>今天的精力状态</legend>
              <div>
                {energy.map((label, i) => (
                  <button
                    key={label}
            ...
            <label className="field">
              ...
              <textarea
                ...
                placeholder="今天推进了什么？哪里有些卡住？也可以只是写下一点感受。"
              />
            </label>
            {data.projects.length > 0 && (
              <div className="review-allocation">
                <h3>
                  项目投入 <span>可选 · 单位为分钟</span>
                </h3>
                {data.projects.map((p) => (
                  <label key={p.id}>
                    <span>{p.name}</span>
          ...
          <div className="review-aside">
            <section className="panel">
              ...
              {review.completed_tasks.length ? (
                ...
                <p className="empty-line">当天还没有完成的任务记录。</p>
              )}
            </section>
            <section className="panel">
              <div className="panel-heading">
              ...
              {review.unfinished_tasks.length ? (
                ...
                <p className="empty-line">没有未完成事项。</p>
              )}
            </section>
  ...
  return (
    <div className="settings-layout">
      <form className="panel settings-card" onSubmit={save}>
        ...
        <p className="form-hint">
          日历与任务时间按此时区记录。未设置具体时间的任务，默认以每日开始时间作为提醒基准。
        </p>
        <button className="button primary" disabled={busy}>
          <Save size={16} />
          {busy ? "保存中…" : "保存设置"}
      ...
      <section className="panel settings-card">
        <div className="panel-heading">
          <div>
            <span className="eyebrow">A GENTLE NUDGE</span>
            <h2>通知与提醒</h2>
          </div>
          <Bell size={20} />
        </div>
        ...
        <div className="info-box">
          <ShieldCheck size={18} />
          <p>
            当前版本在应用打开时检查提醒；关闭网页或系统休眠后，无法保证系统通知准时送达。待触发的提醒会保留，并在下次打开时检查。Google
            Calendar 接口已预留。
          </p>
        </div>
      </section>
      <section className="panel settings-card">
        <div className="panel-heading">
          <div>
            <span className="eyebrow">YOUR DATA, IN YOUR HANDS</span>
            <h2>数据备份</h2>
          </div>
          <Download size={20} />
        </div>
        <div className="settings-feature">
          <div>
            <h3>导出完整数据</h3>
            <p>
              将目标、项目、任务、完成记录与每日复盘导出为 JSON
              文件。数据保存在本机 SQLite 数据库中。
            </p>
          </div>
          <button
            type="button"
      ...
      <section className="panel settings-card">
        ...
        <div className="integration-list">
          <div>
            ...
            <span className="status-pill">接口已预留</span>
          </div>
          <div>
            ...
            <span className="status-pill">尚未启用</span>
          </div>
        </div>
      </section>
    </div>
