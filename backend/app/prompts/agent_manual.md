...
侧边栏在 DayMark 标识下直接显示页面导航，不提供空间切换入口。

- **今日概览**：首屏以循环背景视频展示真实日期、剩余任务数、完成进度、计划时长和进行中项目数；通知、刷新入口和 M 标识在视频顶部，其他页面保留独立顶栏。首页提供跳转本页今日任务区与打开日历的入口。向下滚动后，任务按时间与优先级呈现；侧栏时间表最多显示 4 个设有开始时间的待办任务，点击可编辑；页面还显示昨日未完成、临近截止和整体阶段。独立 Calendar Event 不出现在该任务时间表中。
