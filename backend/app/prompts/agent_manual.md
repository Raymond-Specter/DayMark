...
- **今日概览**：首屏以背景画面展示真实日期、剩余任务、下一项任务、完成进度、计划时长、进行中项目和当前阶段，并提供跳转本页今日任务区与打开日历的入口。向下滚动后，任务按时间与优先级呈现；侧栏时间表最多显示 4 个设有开始时间的待办任务，点击可编辑；页面还显示昨日未完成、临近截止和整体阶段。独立 Calendar Event 不出现在该任务时间表中。
